...
function template_body_above()
{
	...
	<div id="top_section">
		...
		<div class="relative flex items-center h-20 px-4">
		<a class="menu_icon mobile_user_menu"><i class="fas fa-bars"></i></a>
		<div class="navbar-brand flex lg:flex-initial">
		<a id="top" href="', $scripturl, '" class="header-brand__link">';
 
		if(!empty($settings['ft_icon'])) {
		echo ' <span class="header-brand__logo" style="color:', $settings['ft_icon_color'] , '">', $settings['ft_icon'] , '</span>';
				   }
		echo '<span class="header-brand__text t-hack"> ', empty($context['header_logo_url_html_safe']) ? $context['forum_name_html_safe'] : '<img src="' . $context['header_logo_url_html_safe'] . '" alt="' . $context['forum_name_html_safe'] . '">', '</span></a>
		 </div>';
	// Show the menu here, according to the menu sub template, followed by the navigation tree.
	// Load mobile menu here
	echo '
				<div id="main_menu" class="items-stretch h-20 ml-8 space-x-4 lg:flex">
	...
	if ($context['user']['is_logged'])
	{
		...
		echo '
			<ul class="wt_panel right-0 lg:ml-auto lg:static" id="top_info">';

		// Secondly, PMs if we're doing them
		if ($context['allow_pm'])
			echo '
				<li class="item-icon">
					<a href="', $scripturl, '?action=pm"', !empty($context['self_pm']) ? ' class="active"' : '', ' id="pm_menu_top"><i class="fas fa-envelope"></i> ', !empty($context['user']['unread_messages']) ? ' <span class="amt">' . $context['user']['unread_messages'] . '</span>' : '', '</a>
					<div id="pm_menu" class="dropdown-menu-right top_menu"></div>
				</li>';

		// Thirdly, alerts
		echo '
				<li class="item-icon">
					<a href="', $scripturl, '?action=profile;area=showalerts;u=', $context['user']['id'], '"', !empty($context['self_alerts']) ? ' class="active"' : '', ' id="alerts_menu_top"><i class="fas fa-bell"></i> ', !empty($context['user']['alerts']) ? ' <span class="amt">' . $context['user']['alerts'] . '</span>' : '', '</a>
					<div id="alerts_menu" class="dropdown-menu-right top_menu"></div>
				</li>';
				// And now we're done.
		echo '
                               <li>
		...
		echo '
					...
					<div id="profile_menu" class="dropdown-menu-right top_menu"></div>
				</li>';
	...
	{
		echo '
			<ul class="absolute right-0 lg:static lg:ml-auto" id="top_info">
				<li class="visitor">
					<a class="login" href="', $scripturl, '?action=login" onclick="return ', empty($maintenance) ? 'reqOverlayDiv(this.href, ' . JavaScriptEscape($txt['login']) . ')' : 'true', ';"><i class="fas fa-user-check"></i> <span class="header__user-panel-text">', $txt['login'], '</span></a>
				</li>';

		if ($context['can_register'] && empty($maintenance))
			echo '
				<li class="visitor">
					<a class="register" href="', $scripturl, '?action=signup"><i class="fas fa-user-edit"></i> <span class="header__user-panel-text"> ', $txt['register'], '</span></a>
				</li>';
	...
	echo '
		<ul>
			<li class="floatright"><a href="', $scripturl, '?action=help">', $txt['help'], '</a> ', (!empty($modSettings['requireAgreement'])) ? '| <a href="' . $scripturl . '?action=agreement">' . $txt['terms_and_rules'] . '</a>' : '', ' | <a href="#top_section">', $txt['go_up'], ' &#9650;</a></li>
						<li class="copyright">', theme_copyright(), ' <span style="font-size: 1em; color: red;"><i class="fas fa-heart"></i></span> ', Wt_copy(), '</li>
		</ul>';
